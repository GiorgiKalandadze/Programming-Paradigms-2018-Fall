...
static void BuildIndices(const char *feedsFileName, hashset *set, hashset* map);
static void ProcessFeed(const char *remoteDocumentName, hashset *set, hashset* map);
static void PullAllNewsItems(urlconnection *urlconn, hashset *set, hashset* map);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *set, hashset* map);
static void ExtractElement(streamtokenizer *st, const char *htmlTag, char dataBuffer[], int bufferLength);
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL, hashset *set, hashset* map);
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *unused, const char *articleURL, hashset *set, hashset* map);
static void QueryIndices(hashset *set, hashset* map);
static void ProcessResponse(const char *word, hashset *set, hashset* map);
...
static const int DEFAULT_ALLOC_LENGTH = 20;
static const int DEFAULT_NUMBER_OF_TOPS = 10;
//My Variables
static const int NUM_BUCKETS = 10007;
static const char *const stopWordsFileName = "data/stop-words.txt";
...
//My Methods 
void readFile(const char *fileName, hashset *set){
  FILE *infile;
  streamtokenizer st;
  char buffer[1024];
...
//Vector
int VectorCompare(const void *elemAddr1, const void *elemAddr2){
  char* word1 = *(char**)elemAddr1;
  info* info2 = (info*)elemAddr2;
  return strcmp(word1, info2 -> articleURL);
}
void VectorFree(void *elemAddr){
  info* inf = (info*)elemAddr;
  free(inf -> articleName);
  free(inf -> articleURL);
}
...
int CompareFunction(const void *elemAddr1, const void *elemAddr2){
...
void FreeFunction(void *elemAddr){
  free(*(char**)elemAddr);
}

//For second hashset
static int secondHash(const void *s, int numBuckets)  
{            
  int i;
  unsigned long hashcode = 0;
  mp m = *(mp*)s;
...
int secondCompareFunction(const void *elemAddr1, const void *elemAddr2){
  mp m1 = *(mp*)elemAddr1;
  mp m2 = *(mp*)elemAddr2;
  char* s1 = m1.key;
  char* s2 = m2.key;
...
void secondFreeFunction(void *elemAddr){
  mp m = *(mp*)elemAddr;
  char* s = m.key;
  free(s);
  VectorDispose(&m.v);
...
{
  hashset set;
  HashSetNew(&set, sizeof(char*), NUM_BUCKETS, StringHash, CompareFunction, FreeFunction);

  hashset map;
  HashSetNew(&map, sizeof(mp), NUM_BUCKETS, secondHash, secondCompareFunction, secondFreeFunction);

  readFile(stopWordsFileName, &set); 
  setbuf(stdout, NULL);
  Welcome(kWelcomeTextFile);
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &set, &map);
  QueryIndices(&set, &map);
  HashSetDispose(&set);
  HashSetDispose(&map);
  return 0;
}
...
static void BuildIndices(const char *feedsFileName, hashset *set, hashset* map)
{
  ...
  while (STSkipUntil(&st, ":") != EOF) { // ignore everything up to the first selicolon of the line
    ...
    ProcessFeed(remoteFileName, set, map);
  }
...
static void ProcessFeedFromFile(char *fileName, hashset *set, hashset* map)
{
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription, (const char *)fileName, set, map);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew doesn't open one..
  fclose(infile);
...
static void ProcessFeed(const char *remoteDocumentName, hashset *set, hashset* map)
{
  ...
  if(!strncmp(filePrefix, remoteDocumentName, strlen(filePrefix))){
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(filePrefix), set, map);
    return;
  }
  ...
  switch (urlconn.responseCode) {
      case 0: printf("Unable to connect to \"%s\".  Ignoring...", u.serverName);
              break;
      case 200: PullAllNewsItems(&urlconn, set, map);
                break;
      case 301: 
      case 302: ProcessFeed(urlconn.newUrl, set, map);
                break;
...
static void PullAllNewsItems(urlconnection *urlconn, hashset *set, hashset* map)
{
  ...
  while (GetNextItemTag(&st)) { // if true is returned, then assume that <item ...> has just been read and pulled from the data stream
    ProcessSingleNewsItem(&st, set, map);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *set, hashset* map)
{
  ...
  ParseArticle(articleTitle, articleDescription, articleURL, set, map);
}
...
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL, hashset *set, hashset* map)
{
  ...
  switch (urlconn.responseCode) {
      ...
      case 200: printf("Scanning \"%s\" from \"http://%s\"\n", articleTitle, u.serverName);
	        STNew(&st, urlconn.dataStream, kTextDelimiters, false);
		ScanArticle(&st, articleTitle, articleDescription, articleURL, set, map);
		STDispose(&st);
		break;
      case 301:
      case 302: // just pretend we have the redirected URL all along, though index using the new URL and not the old one...
                ParseArticle(articleTitle, articleDescription, urlconn.newUrl, set, map);
		break;
...
//My
void addWordInMap(const char *articleTitle, const char *articleURL, char* wordAddress, hashset *map){
  void* address = HashSetLookup(map, &wordAddress);
  if(address == NULL){
    //Make new
    mp newMp;
    char* word = strdup(wordAddress);
    newMp.key = word;
    VectorNew(&newMp.v, sizeof(info), VectorFree, DEFAULT_ALLOC_LENGTH);
    info inf;
    inf.articleName = strdup(articleTitle);
    inf.articleURL = strdup(articleURL);
    inf.counter = 1;
    VectorAppend(&newMp.v, &inf);
    HashSetEnter(map, &newMp);
  } else {
    mp* m = (mp*) address;
    int index = VectorSearch(&(m -> v), &articleURL, CompareFunction, 0, false);
    if(index == -1){
        info inf;
        inf.articleName = strdup(articleTitle);
        inf.articleURL = strdup(articleURL);
        inf.counter = 1;
        VectorAppend(&(m -> v), &inf);
    } else {
      void* position = VectorNth(&(m -> v), index);
      info* in = (info*)position;
      (in -> counter)++;
    }
  }
}
    ...
    if (strcasecmp(word, "<") == 0) {
      SkipIrrelevantContent(st); // in html-utls.h
        ...
        if (WordIsWellFormed(word)) {
            //Check whether word is in stopWords
            char* wordAddress = word;
            if(HashSetLookup(set, &wordAddress) == NULL){
              addWordInMap(articleTitle, articleURL, wordAddress, map);
            }
            numWords++;
...
static void QueryIndices(hashset *set, hashset* map)
{
  ...
  while (true) {
    ...
    ProcessResponse(response, set, map);
  }
}
//For sort
 int VectorCompareForSort(const void *elemAddr1, const void *elemAddr2){
   info* inf1 = (info*)elemAddr1;
   info* inf2 = (info*)elemAddr2;
   int counter1 = inf1 -> counter;
   int counter2 = inf2 -> counter;
...
{
  if (WordIsWellFormed(word)) {
    void* addre = HashSetLookup(set, &word);
    if(addre == NULL){
      void* address = HashSetLookup(map, &word);
      if(address == NULL){
      ...
      VectorSort(&v, VectorCompareForSort);
      for(int i = 0; i < DEFAULT_NUMBER_OF_TOPS; i++){
        if(VectorLength(&v) <= i) break;
        void* addr = VectorNth(&v, i);
        info* inf = (info*)addr;
        if(inf -> counter > 1) {
            printf("%d.) \"%s\" [search term occurs %d times]\n\"%s\"\n ", i + 1,
                    inf -> articleName, inf -> counter, inf -> articleURL);
          } else{
              printf("%d.) \"%s\" [search term occurs 1 time]\n\"%s\"\n ", i + 1,
                    inf -> articleName, inf -> articleURL);
        }
      }
    } else {
