...
(define (pairs elem list)
    (if (null? list) '()
        (append (intersect elem (car list)) 
                (pairs elem (cdr list))
        )
    )
...
(define (intersection-points list)
    (if (null? list) '()
      (append (pairs (car list) (cdr list)) 
...
(define (distance-product point list)
  (apply *  (map  (lambda (point2) (dist point point2)) 
                  (remove point list)
            )
  )
)

(define (rate-points set)
    (map  (lambda (point) (list (distance-product point set) point)) 
          set
    )
)
...
(define (clumped-points list)
  (apply append (map  (lambda (elem) (cdr elem))
                      (prefix-of-list (sort-points (rate-points list)) (div (length list) 2))
                )
  )
)
...
(define (average-point set)
    (let  ((avg-point (list   (average (map car set))
                              (average (map cadr set))
          )))
          (cons (distance-product avg-point set) (list avg-point))
    )
)

(define (best-estimate list)
  (average-point (clumped-points (intersection-points list)))
)


(define (where-am-i dist stars)
    (sort-points (map best-estimate (all-guesses dist stars)))
)
