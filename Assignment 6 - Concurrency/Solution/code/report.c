...
{
  pthread_mutex_lock(&bank->transfer);
  ...
  if (amountAbs < reportingAmount) {
    pthread_mutex_unlock(&bank->transfer); 
...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  pthread_mutex_lock(&bank->check);
  bank->workers--;
  if(!bank->workers){
    pthread_mutex_unlock(&bank->check);
    /*
    ...
    */
    ...
    pthread_mutex_lock(&bank->next);
    pthread_cond_broadcast(&bank->next_cond);
    pthread_mutex_unlock(&bank->next);
    ...
    pthread_cond_wait(&bank->next_cond, &bank->next);
    pthread_mutex_unlock(&bank->next);
  }
  return 0;
}
